...
def git_commit(name: str, lib_path: pathlib.Path, test_path: pathlib.Path, cpython_dir: pathlib.Path, verbose: bool = True) -> bool:
    ...
    Args:
        ...
        lib_path: Path to library file/directory
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not args.no_commit:
            ...
            # Use Lib/ paths for commit
            commit_lib_path = lib_file_path or pathlib.Path(".")
            commit_test_path = test_path or pathlib.Path(".")
            cpython_dir = get_cpython_dir(original_src)

            git_commit(name, commit_lib_path, commit_test_path, cpython_dir)
