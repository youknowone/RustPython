    ...
    fn compile_statement(&mut self, statement: &ast::Stmt) -> CompileResult<()> {
        ...
        match &statement {
            ...
            ast::Stmt::Assign(ast::StmtAssign { targets, value, .. }) => {
                let direct_name_unpack = (|| {
                    let [target] = &targets[..] else {
                        return None;
                    };
                    let target_elts = match target {
                    ...
                    if target_elts.len() != value_elts.len()
                        || target_elts
                            .iter()
                            .any(|elt| !matches!(elt, ast::Expr::Name(_)))
                        || target_elts
                            .iter()
                            .chain(value_elts.iter())
                            .any(|elt| matches!(elt, ast::Expr::Starred(_)))
                    {
                        return None;
                    }
                    Some((target_elts, value_elts))
                })();

                if let Some((target_elts, value_elts)) = direct_name_unpack {
