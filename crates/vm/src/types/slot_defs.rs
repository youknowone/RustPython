//! Slot definitions array - CPython typeslots.h compatible
//!
//! This module provides a centralized array of all slot definitions,
//! matching CPython's typeslots.h values (1-81).
...
impl SlotAccessor {
    ...
    pub fn get_slot_func_with_op(
        ...
        // For Right operations on binary number slots, use right_* fields
        if op == Some(SlotOp::Right) {
            match self {
                Self::NbAdd => return slots.as_number.right_add.load().map(SlotFunc::NumBinary),
                Self::NbSubtract => {
                    return slots
                        .as_number
                        .right_subtract
                ...
                Self::NbDivmod => {
                    return slots.as_number.right_divmod.load().map(SlotFunc::NumBinary);
                }
                Self::NbPower => {
                    return slots.as_number.right_power.load().map(SlotFunc::NumTernary);
                }
                Self::NbLshift => {
                    return slots.as_number.right_lshift.load().map(SlotFunc::NumBinary);
                }
                Self::NbRshift => {
                    return slots.as_number.right_rshift.load().map(SlotFunc::NumBinary);
                }
                Self::NbAnd => return slots.as_number.right_and.load().map(SlotFunc::NumBinary),
                Self::NbXor => return slots.as_number.right_xor.load().map(SlotFunc::NumBinary),
                Self::NbOr => return slots.as_number.right_or.load().map(SlotFunc::NumBinary),
                Self::NbFloorDivide => {
                    return slots
                        .as_number
                        .right_floor_divide
        ...
        if let Self::TpRichcompare = self {
            if let Some(cmp_op) = op.and_then(|o| o.as_compare_op()) {
                return slots
                    .richcompare
                    .load()
...
mod tests {
    ...
    #[test]
    fn test_slot_accessor_values() {
        // Verify CPython typeslots.h compatibility
        assert_eq!(SlotAccessor::BfGetBuffer as u8, 1);
        assert_eq!(SlotAccessor::MpAssSubscript as u8, 3);
        assert_eq!(SlotAccessor::NbAdd as u8, 7);
        assert_eq!(SlotAccessor::SqAssItem as u8, 39);
        assert_eq!(SlotAccessor::TpCall as u8, 50);
        assert_eq!(SlotAccessor::TpHash as u8, 59);
        assert_eq!(SlotAccessor::TpInit as u8, 60);
        assert_eq!(SlotAccessor::TpNew as u8, 65);
        assert_eq!(SlotAccessor::TpRichcompare as u8, 67);
        assert_eq!(SlotAccessor::NbMatrixMultiply as u8, 75);
        assert_eq!(SlotAccessor::AmSend as u8, 81);
    }
    ...
    fn test_slot_defs_count() {
        ...
        assert!(SLOT_DEFS.len() < 120, "Too many slot definitions");
    }
