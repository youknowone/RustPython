...
impl SealedBag {
    ...
    fn is_expired(&self, global_epoch: Epoch) -> bool {
        ...
        // NOTE: This version of EBR maintain epoch skew of threads ≤ 1 and reclaim garbages
        // three epochs ago.
...
pub(crate) struct Local {
    ...
    pub(crate) manual_count: Cell<usize>,
...
impl Local {
    ...
    pub(crate) unsafe fn defer(&self, mut deferred: Deferred, guard: &Guard) {
        // SAFETY: we have exclusive access via guard
...
impl IsElement<Local> for Local {
    ...
    unsafe fn element_of(entry: &Entry) -> &Local {
        ...
        // SAFETY: caller guarantees entry is from a Local
        unsafe { &*local_ptr }
    }

    unsafe fn finalize(entry: &Entry, guard: &Guard) {
        // SAFETY: element_of is safe per the trait contract
        unsafe {
