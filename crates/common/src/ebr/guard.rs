...
impl Guard {
        ...
        F: FnOnce() -> R,
    {
        ...
        if let Some(local) = unsafe { self.local.as_ref() } {
            unsafe { local.defer(Deferred::new(move || drop(f())), self) };
        } else {
            drop(f());
        }
    }
    ...
    pub(crate) unsafe fn defer_destroy<T>(&self, ptr: RawShared<'_, T>) {
        // SAFETY: caller guarantees ptr is valid and no longer reachable
        unsafe { self.defer_unchecked(move || ptr.drop()) };
    }
...
/// thread pinned - it's just a fake guard that allows loading from atomics unsafely.
///
/// # Safety
///
