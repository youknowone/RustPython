...
pub fn run(init: impl FnOnce(&mut VirtualMachine) + 'static) -> ExitCode {
    ...
    let exitcode = interp.run(move |vm| {
        // Borrow modules from CPython when pyo3 feature is enabled
        #[cfg(feature = "pyo3")]
        rustpython_module_pyo3::borrow_module("_ctypes", vm).expect("borrowing _ctypes failed");

        run_rustpython(vm, run_mode)
    });
